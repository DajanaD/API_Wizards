...
async def delete_comment_route(comment_id: int, db: AsyncSession = Depends(get_db), user: User = Depends(Auth.get_current_user)):
    deleted_comment = await delete_comment(comment_id, db, user)
